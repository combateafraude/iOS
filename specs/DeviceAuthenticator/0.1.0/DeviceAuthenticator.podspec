#
# Be sure to run `pod lib lint CFPassiveFaceLiveness.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'DeviceAuthenticator'
  s.version          = '0.1.0'
  s.summary          = 'Device Authenticator'
  s.description      = <<-DESC
Device Authenticator description
                       DESC

  s.homepage         = 'https://github.com/combateafraude/iOS'
  s.license          = { :type => 'MIT', :file => 'device-authenticator/LICENSE' }
  s.author           = { 'ti@combateafraude.com' => 'ti@combateafraude.com' }
  s.source           = { :git => 'https://github.com/combateafraude/iOS.git', :tag => s.name + "-" + s.version.to_s }
 
  
  s.ios.deployment_target = '12.0'
  s.ios.vendored_frameworks = 'device-authenticator/DeviceAuthenticator.framework'
 
end
